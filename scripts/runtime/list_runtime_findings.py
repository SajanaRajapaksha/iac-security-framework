...
def list_findings(scan_id: str) -> None:
    ...
    print(f"  Prowler Version            : {summary.get('prowler_version', '4.3.4')}")
    print(f"  Scan Scope                 : {summary.get('scan_scope', 'DEPLOYMENT_TAG_FILTER')}")
    print(f"  Manifest Resources         : {summary.get('deployed_resources', 0)}")
    print(f"  Tag-Eligible Resources     : {summary.get('tag_scannable_resources', 0)}")
    print(f"  ARN-Eligible Resources     : {summary.get('arn_scannable_resources', 0)}")
    ...
    print(f"  Deployment Findings        : {summary.get('deployment_findings', 0)}")
    print(f"  Account Context Findings   : {summary.get('account_findings_count', 0)}")
    print(f"  Unmatched Findings         : {summary.get('unmatched_findings_count', 0)}")
    print(f"  Operational Errors         : {summary.get('operational_errors_count', 0)}")
    print("  ----------------------------------------------------------")
